...
class IndianStockTradingEnv(gym.Env):
    ...
    def calculate_reward(self, action, exit_type=None):
        ...
        # Track time spent in drawdown
        if self.net_worth < self.max_net_worth:
            self.time_in_drawdown_steps += 1
            reward -= 0.01  # Small penalty per step in drawdown
        else:
            self.time_in_drawdown_steps = 0
            reward += 0.05  # Small reward for making new equity highs
        ...
        # Stagnation penalty: discourage flat equity curve
        if len(self.net_worth_history) > 20:
            recent_net = self.net_worth_history[-20:]
            if max(recent_net) - min(recent_net) < 0.01 * self.initial_balance:
                reward -= 0.2  # Penalize flat behavior
...
def create_vec_env(df, symbol, **kwargs):
    ...
    def make_env():
        def _init():
            return Monitor(IndianStockTradingEnv(df=df, symbol=symbol, **kwargs))
        return _init
...
def evaluate_model(model, test_df, symbol, num_episodes=1, render=False, save_results=True, **env_kwargs):
    ...
    for episode in range(num_episodes):
        ...
        current_timestamp = test_df.iloc[env.current_step].Date if hasattr(test_df.iloc[env.current_step], 'Date') else f"Step_{env.current_step}"
        ...
        while not done:
            ...
            capital_history.append({
                'episode': episode,
                # 'timestamp': current_timestamp,
                'timestamp': current_timestamp.isoformat() if isinstance(current_timestamp, pd.Timestamp) else str(current_timestamp),
                'capital': env.net_worth,
                'balance': env.balance,
                'shares_held': env.shares_held,
                'shares_value': env.shares_held * current_price if current_price else 0,
            ...
            if shares_change != 0:
                ...
                if trade_type == "BUY":
                    ...
                    holding_periods.append({
                        ...
                        'exit_time': current_timestamp.isoformat() if isinstance(current_timestamp, pd.Timestamp) else str(current_timestamp),
        ...
        if save_results:
            ...
            with open(metrics_path, 'w') as f:
                ...
                json.dump(sanitize_for_json(sanitized_metrics), f, indent=4)
    ...
    if save_results:
        ...
        with open(avg_metrics_path, 'w') as f:
            json.dump(sanitize_for_json(avg_metrics), f, indent=4)
    ...
    for symbol in SYMBOLS:
        ...
        if os.path.exists(model_path):
            ...
            with open(f"{symbol}_best_hyperparameters.json", "w") as f:
                json.dump(sanitize_for_json(best_hyperparameters), f, indent=4)
            ...
            for i in range(0, total_timesteps, batch_size):
                ...
                best_model.learn(total_timesteps=min(batch_size, total_timesteps - i), reset_num_timesteps=False, progress_bar=True, log_interval=log_interval)
                end_time = time.time()
        ...
        if bh_metrics is not None:
                ...
                with open(bh_metrics_path, 'w') as f:
                    json.dump(sanitize_for_json(bh_metrics), f, indent=4)
        ...
        if detailed_info['trades']:
            ...
            trade_table = []
            trade_headers = ["Date", "Action", "Price (₹)", "Shares", "Value (₹)", "Capital After (₹)", "Stop Loss (₹)", "Target (₹)"]
            
            for trade in detailed_info['trades']:
                trade_row = [
                    datetime.fromisoformat(trade['timestamp']).strftime('%d-%m-%Y') if 'T' in trade['timestamp'] else trade['timestamp'],
                    trade['action'],
        ...
        if detailed_info['holding_periods']:
            print("\nHolding Periods:")
            holding_table = []
            holding_headers = ["Entry Time", "Exit Time", "Holding Period", "Entry Price (₹)","Exit Price (₹)", "Shares", "P/L (₹)", "P/L %", "Exit Reason"]
            
            for period in detailed_info['holding_periods']:
                entry_time = (
                    period['entry_time'].strftime('%d-%m-%Y')
                    if isinstance(period['entry_time'], pd.Timestamp)
                    else datetime.fromisoformat(period['entry_time']).strftime('%d-%m-%Y')
                    if isinstance(period['entry_time'], str)
                    else str(period['entry_time'])
                )

                exit_time = (
                    period['exit_time'].strftime('%d-%m-%Y')
                    if isinstance(period['exit_time'], pd.Timestamp)
                    else datetime.fromisoformat(period['exit_time']).strftime('%d-%m-%Y')
                    if isinstance(period['exit_time'], str)
                    else str(period['exit_time'])
                )
