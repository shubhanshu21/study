...
class IndianStockTradingEnv(gym.Env):
    ...
    def _calculate_dynamic_stop_loss(self, price, position_type):
        ...
        if position_type == 'long':
            stop_loss = price - (current_atr * self.trailing_stop_atr_multiplier)
        else:  # short position
            stop_loss = price + (current_atr * self.trailing_stop_atr_multiplier)

        # logger.log(LOG_LEVELS['DATA_LOAD'], f"Dynamic stop_loss calculation complete. {stop_loss}.")    
        return stop_loss
    ...
    def _check_stop_and_target(self, current_price):
        """Check if price hit stop loss or target"""
        if not self.position_type:
            return False, None
